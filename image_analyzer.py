...
def analyze_image(image_path, api_key):
    ...
    try:
        ...
        try:
            ...
            # 构建提示词，包含用户要求的所有反推要点
            prompt = """在完全遵循图片内容的情况下，直接返回图片描述内容。生成一段 300 字以内类似下面这段提示词的提示词，并且注重描述画面的真实感，需要描述画面风格（如真实人像摄影、中国水墨画，油画，水彩画等）、画面中光线或明暗程度，镜头角度，画面构图比例等，如果是人脸脸部特写镜头需要描述人物表情，皮肤细节，人物性别国籍，如果是全身则需要描述清楚人物的姿势造型，如果是中国画等绘画作品需要描述具体的画种风格，这些描述都需要用自然语言回答，不要出现精准的数字，不允许出现特殊符号。
## 原则
    1. 使用**客观、精确和基于事实的语言**。
